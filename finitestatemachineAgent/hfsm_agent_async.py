...
class RouterState(AsyncHierarchicalState):
    ...
    async def handle(self, context: AsyncExecutionContext):
        ...
        if (self.enable_parallel and not parallel_checked and is_root):
            ...
            return self.find_state_by_type("ParallelPlanningState")
...
class ParallelPlanningState(AsyncHierarchicalState):
    ...
    async def handle(self, context: AsyncExecutionContext):
        ...
        if plan.branches and len(plan.branches) > self.max_branches:
            logger.warning(f"⚠️ [ParallelPlan] Too many branches ({len(plan.branches)}), limiting to {self.max_branches}")
            plan.branches = plan.branches[:self.max_branches]
        
        # Store plan in context
        # Use .dict() for Pydantic serialization safety
        await context.set_memory("parallel_plan", plan.dict())
        ...
        if plan.strategy == "parallel_research" and plan.branches:
            ...
            logger.info(f"➡️ [ParallelPlan] Single path execution")
            return self.find_state_by_type("RouterState")
